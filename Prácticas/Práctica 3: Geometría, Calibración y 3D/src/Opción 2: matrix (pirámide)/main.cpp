...
// create3DPyramid() function
// Generates the 3D coordinates of the 5 vertices of the pyramid
std::vector<cv::Point3f> 
create3DPyramid(float baseSize = 96.0f, float height = -144.0f)
{
    ...
    pyramid.push_back(cv::Point3f(centerX - halfSize, centerY - halfSize, 0));
    pyramid.push_back(cv::Point3f(centerX + halfSize, centerY - halfSize, 0));
    pyramid.push_back(cv::Point3f(centerX + halfSize, centerY + halfSize, 0));
    pyramid.push_back(cv::Point3f(centerX - halfSize, centerY + halfSize, 0));
    pyramid.push_back(cv::Point3f(centerX, centerY, height));
    return pyramid;
}
...
main()
{
    ...
    std::vector<cv::Point3f> pyramid3D = create3DPyramid();
    ...
    if (found) {
        ...
        for (int i = 0; i < 4; i++) {
            cv::line(image, pyramid2D[i], pyramid2D[4], cv::Scalar(0, 0, 255), 2);
        }
